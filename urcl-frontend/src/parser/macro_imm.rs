...
impl<'a> Parser<'a> {
    ...
    pub(crate) fn bits_umsb(&self) -> Natural {
        bits_vals(self.bits().saturating_sub(1) as _)
    }

    pub(crate) fn bits_smax(&self) -> Natural {
        bits_max(self.bits().saturating_sub(1) as _)
    }

    pub(crate) fn bits_smsb(&self) -> Natural {
        bits_vals(self.bits().saturating_sub(2) as _)
    }

    pub(crate) fn bits_uhalf(&self) -> Natural {
        self.bits_umax() ^ self.bits_lhalf()
    }

    pub(crate) fn bits_lhalf(&self) -> Natural {
        bits_max((self.bits() as usize + 1) / 2)
    }
}
